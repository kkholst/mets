...
render_toc <- function(
  ...
  x <- sapply(x, function(h) {
    ...
    if (grepl("\\{#.+\\}(\\s+)?$", h)) {
      ...
      # GitHub slug: lowercase, strip punctuation (keep letters/digits/space/-/_),
      # collapse whitespace, convert spaces to hyphens
      header_slug <- tolower(header_text)
      header_slug <- gsub("[^[:alnum:] _-]", "", header_slug)
      header_slug <- gsub("\\s+", "-", header_slug)
    }
...
```{r toc, echo=FALSE}
render_toc("README.Rmd", toc_depth = 3)
```
...
> Its Application 9, pp. 47-69. doi: <http://dx.doi.org/10.1146/annurev-statistics-040120-024253>
...
Consequently, we can also compute Brier-score for RMST and RMTL regression for survival and competing risks data using the
outcome model specification for the binreg function (see examples). 

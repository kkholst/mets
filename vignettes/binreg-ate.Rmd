...
\end{align*}
the estimation is based on IPCW adjusted estimating equation (EE)
\begin{align*}
...
The two score equations are quite similar, and exactly the same when the censoring model is fully-nonparametric given $X$.
 ...
 - It seems that the binreg estimating equations most often the preferable ones to use, and the estimating equation used
   is augmented in the default implementation (see the binreg vignette).
...
under known censoring model. The influence functions are stored in the output. The standard errors can be
...
where
 ...
 large censorng model as we also illustrate below. 
   ...
   - Censoring model can be specified by strata (used for phreg
...
For large data where there are more than 2 treatment groups the computations can be memeory extensive when
there are many covariates due to the multinomial-regression model used for the propensity scores. Otherwise
the function (binregATE) will run for rather large data and be rather fast.

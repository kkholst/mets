...
e.wglm <- wglm( Surv(time,cause)~gp.f+dnr+preauto+ttt24, 
	       formula.censor = ~+1, 
	       times = 60, data = kumar, product.limit=TRUE,cause=2)
summary(e.wglm)$coef
estimate(ibs)

es.wglm <- wglm( Surv(time,cause)~gp.f+dnr+preauto+ttt24, 
formula.censor = ~strata(gp,dnr), times = 60, 
...
summary(b3)
#> Regression: outcome= numeric link-model= logit 
#>    n events
#>  400    400
#> 
#>  400 clusters
...
## same as, 
bb3 <- binregATE(cause2~gp.f+dnr+preauto+ttt24,kumar,treat.model=gp.f~dnr+preauto+ttt24,model="logit")
summary(bb3)
#> Regression: outcome= numeric link-model= logit 
#>    n events
#>  400    400
...
## same as 
###dfactor(kumar) <- cause2f~cause2
###bb3 <- binregATE(cause2f~gp.f+dnr+preauto+ttt24,kumar,treat.model=gp.f~dnr+preauto+ttt24)
###summary(bb3)
...
summary(b3)
#> Regression: outcome= numeric link-model= lin 
#>    n events
#>  400    400
#> 
#>  400 clusters
...
## same as, 
bb3 <- binregATE(time~gp.f+dnr+preauto+ttt24,kumar,treat.model=gp.f~dnr+preauto+ttt24)
summary(bb3)
#> Regression: outcome= numeric link-model= lin 
#>    n events
#>  400    400
...
#> [25] RcppArmadillo_15.4.2-1 globals_0.19.1         survival_3.8-11
```

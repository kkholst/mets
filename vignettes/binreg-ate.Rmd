...
The binregATE function can estimate the average treatment treatment effect of binary outcomes with 
IPCW adjustment for a specific time-point, 
...
A nice feature is that influence functions are computed and are available.
...
We here generate similar data by assuming that the two cumulative incidence curves are logistic and we
...
We start by computing the average treatment effects for the transplant periods (gp) to get started, and then consider 
...
of gp and dnr. We later return to  a more detailed explanation of the setting with the key components being

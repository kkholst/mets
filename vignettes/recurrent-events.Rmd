...
rc <- exp( X %*% c(0,0))
fz <- NULL
rr <- mets:::simGLcox(n,base1,ddr,var.z=1,r1=r1,rd=rd,rc=rc,fz,cens=3/5000,type=2) 
rr <- cbind(rr,X[rr$id+1,])
...
```{r}
 ...
 outs <- phreg(Event(start,stop,statusD==1)~X1+X2+cluster(id),data=rr)

 tsout <- twostageREC(outs,out,data=rr,ghosh.lin=1,model="shared",theta=1,
		      nu=-lava::logit(0.5))
 summary(tsout)
```

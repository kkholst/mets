...
legend("bottomright",c("Eff-pred"),lty=2:3,col=c(1,3))
...
One can also do regression model, using the model
\begin{align*}
...
```{r}
 rr <- mets:::simMarginalMeanCox(1000,cens=3/5000,Lam1=base1,LamD=ddr,beta1=c(0.3,-0.3),betad=c(-0.3,0.3))
 ...
 outs <- recreg(Event(start,stop,statusG)~X1+X2+cluster(id),data=rr,cause=1,death.code=3,cens.code=0,cens.model=~strata(X1,X2))
...
recEFF1x <- recurrentMarginalAIPCW(rr,times,start="start",stop="stop",status="status",death="death",cause=1,x.design=c("X1","X2"),terms=NULL)
with( recEFF1x, cbind(muPAt,semuPAt,semuPAt/semuP))

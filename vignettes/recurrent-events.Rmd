...
quantities to get some basic understanding of the phenomenon studied. 
...
* Dependence=3: We can draw gamma distributed random effects $Z_1,Z_2,Z_d$ were the sum-structure can be specified via a matrix
...
\end{align}
see Cook & Lawless (1997) and  Gosh & Lin (2000). 
...
dats <- mets:::simGLRA(100,base1,drcumhaz,varz=1)
datsA <- dats[[1]]
...
We get two data-sets, one with only administrative censuring datsA 
and one with additional random censuring datsRA. 
 
  - datsA: statusD has events (1) death (3) and administrative censurings at the  time censorA
  - datsRA: statusD has events (1) death (3),  administrative censurings (7) and random censurings (0). The time 
     of the administriative censuring is given by censorA
...
```{r}
    ## std Right censoring  on combined censuring time 
...
rr <- mets:::simGLcox(n,base1,dr,var.z=1,r1=r1,rd=rd,rc=rc,cens=1/5000,type=3) 
dtable(rr,~statusD)
rr <- cbind(rr,X[rr$id+1,])
###
...
We use the estimator of the probabilty  of exceeding "k" events based on the fact that 

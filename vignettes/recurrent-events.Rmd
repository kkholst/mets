...
```{r}
 out  <- recreg(Event(start,stop,statusG)~X1+X2+cluster(id),data=rr,cause=c(1,3),death.code=3,cens.code=0)
 summary(out)$coef
```
...
E(N_1(t) + I(D<t,\epsilon=3) | X) &  = \Lambda_0(t)  \exp(X^T \beta)
\end{align*}
and with weights that follow the causes, here 1 and 2. 
...
```{r}
 ...
 outC  <- recreg(Event(start,stop,statusGC)~X1+X2+cluster(id),data=rr,cause=c(1,3),death.code=c(3,4),cens.code=0)
 summary(outC)$coef

 outCW  <- recreg(Event(start,stop,statusGC)~X1+X2+cluster(id),data=rr,cause=c(1,3),death.code=c(3,4),cens.code=0,wcomp=c(1,2))
 summary(outCW)$coef

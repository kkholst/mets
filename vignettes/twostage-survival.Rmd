...
```{r}
 ...
 data.fam <- data[mm$familypairindex,]
 data.fam$subfam <- mm$subfamilyindex
 data.fam$cluster <- mm$clusters

 data.fam.clust <- fast.reshape(data.fam[,c("subfam","type")],id="subfam")

 tdp <-transform(data.fam.clust,tt=interaction(type1,type2))
 tdp$tt <- droplevels(tdp$tt)
 dtable(tdp,~tt)
 tmm <- model.matrix(~-1+tt,tdp)
 
 theta.des <- tmm[mm$subfamilyindex+1,]
...
se.clust <- data.fam$cluster
pa <- phreg(Surv(time,status)~cluster(cluster),data.fam)

tsp <- twostage(pa,data=data.fam,theta.des=theta.des,pairs=pairs,se.clusters=se.clust)
summary(tsp)
```

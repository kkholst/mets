...
While the semi-parametric nature of the Cox-model yields a lot of flexibility to
mimic real data in simulations, it is sometimes convenient to have a simulation
model that is fully parametric.

Here we will consider Weibull models parametrized such that the  cumulative hazard given by $\Lambda(t) = \lambda \cdot
 t^s$
where $s$ is the **shape parameter**, and $\lambda$ the **rate parameter**. We
 will allow for a regression model on both parameters
\begin{align*}
 \lambda := \exp(\beta^\top X)},  \quad s := \exp(\gamma^\top Z)
 \end{align*}
given covariates $X, Z$. 

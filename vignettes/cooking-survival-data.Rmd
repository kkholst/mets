...
vignette: >
  %\VignetteIndexEntry{Cooking Survival Data: 5-Minute Recipes}
...
<img src="figure/cooking-survival-data-unnamed-chunk-4-1.png" alt="Cumulative hazard: simulated vs fitted Cox model."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-4-2.png" alt="Cumulative hazard: simulated vs fitted Cox model."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-5-1.png" alt="Cumulative hazard: competing risks Cox models, two causes."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-6-1.png" alt="Cumulative hazard: stratified nonparametric model."  />
...
<img src="figure/cooking-survival-data-stratified-1.png" alt="Cumulative hazard: stratified competing risks models."  />
...
<img src="figure/cooking-survival-data-stratified-2.png" alt="Cumulative hazard: stratified competing risks models."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-7-1.png" alt="Cumulative hazard: fully nonparametric competing risks."  />
...
<img src="figure/cooking-survival-data-entry-stratified-1.png" alt="Cumulative hazard: delayed entry competing risks."  />
...
<img src="figure/cooking-survival-data-weibull1-1.png" alt="Cumulative hazard: Weibull vs Cox stratified model."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-9-1.png" alt="Multistate model: estimated vs true transition rates."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-10-1.png" alt="Cumulative incidence: logistic and cloglog link models."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-11-1.png" alt="Cumulative incidence: fitted model vs simulated data."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-12-1.png" alt="Cumulative incidence: single cause with delayed entry."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-13-1.png" alt="Cumulative incidence: two causes with delayed entry."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-14-1.png" alt="Cumulative incidence: parametric baselines with delayed entry."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-15-1.png" alt="Recurrent events: two-stage Cox model."  />

With our own covariates 


``` r
 X <- matrix(rbinom(2*n,1,0.5),n,2)
 beta1 <- c(0.3,-0.3)
 betad <- c(0.3,-0.3)
 r1 <- exp( X%*% beta1)
 rd <- exp( X%*% beta1)
 simccO <- sim_recurrent_ts(xr,dr,n=n,r1=r1,rd=rd,cens=0.25)
 simccO$X1 <- X[simccO$id,1]
 simccO$X2 <- X[simccO$id,2]

 xrs <- phreg(Surv(entry,time,status==1)~X1+X2+cluster(id),data=simccO)
 drs <- phreg(Surv(entry,time,status==3)~X1+X2+cluster(id),data=simccO)
 estimate(xrs)
#>    Estimate Std.Err    2.5%   97.5% P-value
#> X1   0.1409  0.1824 -0.2166  0.4984 0.43974
#> X2  -0.5093  0.1816 -0.8652 -0.1535 0.00503
 estimate(drs)
#>    Estimate Std.Err    2.5%   97.5%  P-value
#> X1  -0.2279  0.3055 -0.8268  0.3709 0.455630
#> X2  -0.8256  0.3181 -1.4492 -0.2021 0.009453

 par(mfrow=c(1,2))
 plot(xrs); 
 plot(xr,add=TRUE,col=3,lwd=3)
###
 plot(drs)
 plot(dr,add=TRUE,col=3,lwd=3)
```

<img src="figure/cooking-survival-data-unnamed-chunk-16-1.png" alt="Recurrent events: two-stage-II Cox model."  />
...
#> treatment1  -0.4301  0.1831 -0.7889 -0.07132  0.0188
 ...
 estimate(GLs)
#>            Estimate Std.Err    2.5%  97.5% P-value
#> treatment1 -0.04038  0.1476 -0.3296 0.2489  0.7844
 estimate(drs)
#>            Estimate Std.Err   2.5%  97.5% P-value
#> treatment1  -0.5098  0.3211 -1.139 0.1196  0.1124
...
<img src="figure/cooking-survival-data-unnamed-chunk-17-1.png" alt="Recurrent events: Ghosh-Lin and Cox model."  />
...
estimate(xr)
#>     Estimate Std.Err     2.5%   97.5% P-value
#> age -0.01162 0.04044 -0.09087 0.06764  0.7739
estimate(rxr)
#>     Estimate Std.Err    2.5% 97.5% P-value
#> age -0.00045 0.05686 -0.1119 0.111  0.9937
estimate(dr)
#>     Estimate Std.Err    2.5%    97.5% P-value
#> x   -0.43284 0.18302 -0.7916 -0.07412 0.01803
#> age  0.09625 0.08768 -0.0756  0.26810 0.27232
estimate(rdr)
#>     Estimate Std.Err    2.5%  97.5%  P-value
#> x   -0.06649  0.1960 -0.4507 0.3177 0.734449
#> age  0.31483  0.1002  0.1185 0.5112 0.001672
plot(xr); plot(rxr,add=TRUE)
plot(dr,add=TRUE,col=2); plot(rdr,add=TRUE,col=2)
```

<img src="figure/cooking-survival-data-unnamed-chunk-18-1.png" alt="Recurrent events: stratified two-stage model."  />
...
estimate(rxr)
#>     Estimate Std.Err    2.5%   97.5% P-value
#> age -0.08981 0.05505 -0.1977 0.01809  0.1028
estimate(dr)
#>     Estimate Std.Err    2.5%    97.5% P-value
#> x   -0.43284 0.18302 -0.7916 -0.07412 0.01803
#> age  0.09625 0.08768 -0.0756  0.26810 0.27232
estimate(rdr)
#>     Estimate Std.Err   2.5%   97.5% P-value
#> x   -0.52331 0.21209 -0.939 -0.1076 0.01361
#> age  0.03716 0.09343 -0.146  0.2203 0.69083
plot(glr); plot(rxr,add=TRUE)
plot(dr,add=TRUE,col=2); plot(rdr,add=TRUE,col=2)
```

<img src="figure/cooking-survival-data-unnamed-chunk-18-2.png" alt="Recurrent events: stratified two-stage model."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-19-1.png" alt="Recurrent events: frailty model simulations."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-19-2.png" alt="Recurrent events: frailty model simulations."  />
...
<img src="figure/cooking-survival-data-unnamed-chunk-19-3.png" alt="Recurrent events: frailty model simulations."  />
...
#> [10] evaluate_1.0.5         numDeriv_2016.8-1.1    yaml_2.3.12           
#> [13] mvtnorm_1.4-2          timereg_2.0.7          compiler_4.5.2        
#> [16] codetools_0.2-20       Rcpp_1.1.2             future_1.75.0         

...
```
 ...
 - sim_phreg for phreg, can deal with strata 
 - sim_phregs cause specific hazards on phreg form 
...
library(mets)
 ...
 simcoxcox <- sim_recurrent(xr,dr,n=n,data=hf)
...
```{r}
 ...
 simglcox <- sim_recurrent(recGL,dr,n=n,data=hf)
...
n <- 400 
rr <- sim_recurrent(xr,dr,n=n,data=hf)
...
dr <- phreg(Surv(entry,time,status==2)~x+strata(Z1)+age+cluster(id),data=hf)
n <- 400
rr <- sim_recurrent(glr,dr,n=n,data=hf)
...
```{r}
 ...
 drl <- list(dr,base4)
 rr <- sim_recurrentList(n,cumhaz,death.cumhaz=drl)
 dtable(rr,~death+status)

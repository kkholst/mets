...
covariate effects via proportional hazards models by giving HR terms for all components as
well as the exponential censoring. Certain dependencies can be generated by specifying the type of 
...
$F_2(\tau,X)/(F_1(\tau,X)+F_2(\tau,X))$ to decide if it is a cause 1, $\epsilon=1$,  $\epsilon=1$, or a cause 2, $\epsilon=2$. 
...
The function simul.cifs
 ...
 - the censoring is exponential with rate rc=0.5 and can be made covariate dependent (dependence=1) with covariate effects given by
   rcZ, thus with rate $rc \cdot  \exp(Z^T rcZ)$, and dimensions must match. Then censoring can be omitted by $rc=NULL$.

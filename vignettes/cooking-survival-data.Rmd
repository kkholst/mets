...
```{r}
 ...
 coxss <- phreg(Surv(time,status==9)~strata(vf,chf)+diabetes+wmi,data=sTRACE)
 dd <- sim.phreg(coxss,100,data=sTRACE)
 scoxs <- phreg(Surv(time,status)~strata(vf,chf)+diabetes+wmi,data=dd)
 plot(scoxs,col=2); plot(coxss,add=TRUE,col=1)
```

...
\node[box, right of=start, yshift=-2.4cm] (lt) {terminal};

% Arrows with hazard labels
\draw[->, thick] (start) -- node[above] {$F_{exceed-k}(t)$} (death);
\draw[->, thick] (start) -- node[below] {$F_{terminal}(t)$} (lt);
\end{tikzpicture}
...
```{r}
     ...
     RecLost <- recregIPCW(Event(entry,time,status)~-1+treatment+cluster(id)+marks(lost5),data=hf,cause=1,death.code=2,time=5,
			 cens.model=~strata(treatment),model="lin")
     summary(RecLost)
     head(iid(RecLost))
```

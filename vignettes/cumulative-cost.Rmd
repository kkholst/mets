...
\end{align*}
where $N(s)$ is a counting process that registers the times at which the cost is realized and accumulated, 
and $Z(t)$ is the cost (or marks) at the event times. The counting process can be 
...
\begin{align*}
 P ( {\cal U}(t)  > k ) & = \mu_k(t),
\end{align*}
and in the situation with a terminal this is based on a derived competing risks data that 
...
Regression modelling of this quantity is also possible using competing risks regression models, using for example, the 

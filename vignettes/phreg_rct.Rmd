...
The times of randomization is specified by 
   ...
   - weight.var  is "1" when a randomization is given 
...
```{r}
###library(speff2trial) 
###dfactor(ACTG175) <- arms.f~arms
###save(ACTG175,file="ACTG175.rda")
library(mets)
data(ACTG175)
###
...
# Prop Haz  -0.70583  0.15379  -1.0073  -0.40441  4.4411e-06
# Speff     -0.76083  0.15189  -1.0585  -0.46313  5.4705e-07
...
fit2 <- phreg_rct(Event(start,stop,statusD)~Z.f+cluster(id),data=data,
	 weight.var="treattime",typesR=c("non","R0"),typesC=c("non","C","dynC"),
...
sse <- phreg_rct(Event(start,time,statusD)~A0.f+A1t+cluster(id),data=rr,
	 typesR=c("non","R0","R1","R01"),typesC=c("non","C","dynC"),weight.var="treattime",

...
\begin{align}
\log(-\log( \frac{1-F_1(t,X_1+1,X_2)}{1-F_1(t,X_1,X_2)})) & =  \beta_1,
\end{align}
...
### cbind(cr$coef,diag(cr$var)^.5,fg$coef,fg$se.coef)
#           [,1]       [,2]        [,3]       [,4]
# Z1  0.06882201 0.08184642  0.06882199 0.08184642
# Z2 -0.36807861 0.16606441 -0.36807951 0.16606444

### fg0 <- cifreg(Event(time,status)~Z1+Z2,data=dats,cause=1,propodds=NULL,beta=cr$coef,no.opt=TRUE)
### cbind(diag(cr$var)^.5,fg0$se.coef)
#            [,1]       [,2]
# [1,] 0.08184642 0.08184642
# [2,] 0.16606441 0.16606441
```

Estimates are slightly different, but standard errors seems to agree. 
...
###cbind(cr$coef,diag(cr$var)^.5,cifnc$coef,cifnc$se.coef)
#           [,1]       [,2]        [,3]       [,4]
# Z1  0.06785579 0.08251841  0.06785579 0.08251841
# Z2 -0.37911615 0.16716486 -0.37911703 0.16716489
```
...
\begin{align*} 
	q(t) & =   E( H(t,X) I(T  \leq t, \epsilon=2)/G_c(T)) = E( H(t,X) F_2(t,X) ), 
\end{align*}
...
\begin{align*} 
	q(t,X) & =   E( H(t,X) I(T  \leq t, \epsilon=2)/G_c(T) | X) = H(t,X) F_2(t,X), 
\end{align*}
...
The function $FG_AugmentCifstrata$ 
...
(assuming that the baseline and censoring weights are known), but
works rather well in simulations. In the ${{\bf timereg}}$-package the model can be 
fitted using different estimators that are computed using the correct influence functions but this is 
much slower.  

...
The baseline can also be written as a sum of iid terms and for fixed t this iid decomposition can be
obtained and used for constructing confidence intervals for predictions 
...
```{r}
 ...
 Biid <- mets:::iid.baseline.cifreg(fg,time=3)
 head(Biid)
```

Now using the iid decomposition of the baseline together with iid decomposition of 
$\hat \beta - \beta_0$ we can construct confidence intervals for predictions 

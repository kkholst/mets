...
The cifreg function can fit the Fine-Gray model and the logit-link cumulative incidence function for the 
cause of interest for competing risks, and is completely scalable, that is, linear in the data. 
In addition, for the Fine-Gray model predictions can be provided standard errors for specific time-points based on 
influence functions for the baseline and the regression coefficients.  
...
\cite{fine:gray:1999} considered a cumulative incidence on the form 
\begin{align*}
 F_1(t,X) & = P(T \leq t, \epsilon=1) = 1 - exp( - \Lambda_0(t) \exp(X^T \beta)).
\end{align*}

In the case of independent right-censoring with the censoring distribtion $G_c(t,X) = P(C > t | S(X))$ where
...
The advantage of the Fine-Gray model is that it is easy to fit and it is easy to get standard errors, 
and that it is quite flexible. On the downside 
is that the coefficients must be interpreted on $cloglog$-scale. 
Specifically, 
\begin{align}
...
decompostion (influence functions) of the estimator that we give later. A similar decomposition
...
The iid decompositions are stored inside Biid, in addition we note that the 
iid decompositions for $\hat \beta - \beta_0$ are obtained by the command iid()
...
times for the Fine-Gray or logistic link cumulative incidence regression models. 
 ...
 - when a administrative censoring time is given the risk-set will be increased for all those experiencing the alternative event. 
 - if there are also additional random censorings these can be adjusted for via IPCW weights and are then found using the cens.code 
 - the alternative causes are found by considering cause, cens.code and total number of values for the status variable, so it can be useful to specify the other death.code (s) also
...
phreg/coxph function (thus equivalent to ccA)

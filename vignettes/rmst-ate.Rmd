...
The resmeanATE or resmeanIPCW/rmstIPCW functions can fit an exponential/linear link model with 
IPCW adjustment for a specific time-point for the restricted mean survival or years lost for 
competing risks. 
The function can be used for large data and is completely scalable, that is, linear in the data. 
The function can thus be used for large data. 
...
get the RMST (with the resmean_phreg function)
\[ \int_0^t S(s|X) ds \].

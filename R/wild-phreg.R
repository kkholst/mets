...
if (gplot==1) {
 ...
 ##  pcumsdb.EE.log.o <- timereg::percen(apply(abs(zlogcums),2,max, na.rm=TRUE),0.95)
    pcumsdb.EE <- quantile(c(apply(abs(zcums),2,max, na.rm=TRUE)),probs=0.95)
    pcumsdb.EE.log <- quantile(apply(abs(zcums),2,max, na.rm=TRUE),probs=0.95)
    pcumsdb.EE.log.o <- quantile(apply(abs(zlogcums),2,max, na.rm=TRUE),probs=0.95)

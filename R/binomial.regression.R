...
##'
##' The outcome can also be numeric or a factor and then the estimatingestimating equations are solved with this response without IPCW adjustments. 
##' Finally, we may also specify Ydirect that then are used for solving the equations wiht IPCW adjusment.
##' @section References:
##' \itemize{
...
{# {{{
  ...
  if (is.null(des.weights)) {
    ...
    if (inherits(Y, c("Event", "Surv"))) { ## {{{  Event time outcome 
	 ...
	 if (model[1]=="default") {
	    ...
	    Yf <- FALSE; nY <- 1
            if (is.factor(Y)) { nY <- nlevels(Y); Y <- as.numeric(Y)-1; outcome <- "factor"; Yf <- TRUE} else outcome <- "numeric"
	 ...
	 ## default links
	 if (model[1]=="default") {
		 if (Yf) { if (nY==2) model <- "logit" } else model <- "lin"
	 }
    } ## }}} 
 ...
 if (se) {## {{{ censoring adjustment of variance  and infinitissimal jacknife pseudo augmentation
...
###    h  <-  apply(X*Y,2,revcumsumstrata,xx$strata,xx$nstrata)
    h  <-  Mrevcumsumstrata(X*Y,xx$strata,xx$nstrata)
...
###    IhdLam0 <- apply(h*S0i2*btime,2,cumsumstrata,xx$strata,xx$nstrata)
    ...
    mid <- max(id)
###    MGCiid <- Msumstrata(MGt,2,sumstrata,xx$id,mid+1)
...
###    MGtiid <- apply(MGt,2,sumstrata,xx$id,mid+1)
    MGtiid <- Msumstrata(MGt,xx$id,mid+1)
    augmentation  <-  colSums(MGtiid) + augmentation
    ###
    EXt  <-  Mrevcumsumstrata(X,xx$strata,xx$nstrata)
    IEXhYtdLam0 <- Mcumsumstrata(EXt*c(hYt)*S0i*S0i2*btime,xx$strata,xx$nstrata)
    U <- matrix(0,nrow(xx$X),ncol(X))
    U[xx$jumps+1,] <- (resC$jumptimes<time)*hYt[xx$jumps+1]*EXt[xx$jumps+1,]/c(resC$S0)^2
    MGt2 <- (U[,drop=FALSE]-IEXhYtdLam0)*c(xx$weights)
    ###
    MGCiid2 <- Msumstrata(MGt2,xx$id,mid+1)
...
lp <- c(X %*% pp+offset)
	...
	p <- expit(lp)
        if (monotone) {
           ...
           varp <- p/(1+exp(lp))
           Dlogl <- weights*X*c((Y-p)*varp)
           D2logl <- c(weights*varp^2)*X2
	}
} else stop("link functions must be logit,exp,lin\n") 
ploglik <- sum(weights*(Y-p)^2)
## to avoid using ploglik for fitting, with exp,logit only solve estimating equation
if (model[1]!="lin") ploglik <- 0

###D2log <- apply(D2logl,2,sum)
###gradient <- apply(Dlogl,2,sum)+augmentation
D2log <- colSums(D2logl)
gradient <- colSums(Dlogl)+augmentation
np <- length(pp)
...
###      beta.iid <-  apply(beta.iid,2,sumstrata,id,max(id)+1)
      beta.iid <-  Msumstrata(beta.iid,id,max(id)+1)
...
###	      if (!se) return(cc)
  ...
  val <- c(val,list(time=time,formula=formula,formC=formC,
    ...
    n=nrow(X),nevent=nevent,ncluster=nid))
...
##'
##' The outcome can also be numeric or a factor and then the estimating equations are solved with this response without IPCW adjustments. 
##' @section References:
##' \itemize{
##'   \item Blanche PF, Holt A, Scheike T (2022). "On logistic regression with right censored data, with or without competing risks, and its use for estimating treatment effects." \emph{Lifetime Data Analysis}, 29, 441–482.
##' }
...
## {{{  reading design 
 ...
 data$id__ <- id 
  ...
  if (inherits(Y, c("Event", "Surv"))) { ## {{{ setting up status, response, censoring model 
    if (ncol(Y) == 2) {
        exit <- Y[, 1]
        entry <- rep(0, nrow(Y))
        status <- Y[, 2]
...
## change id from call to id__ to get ordering as data
...
datA <- dkeep(data,x=all.vars(formulanc))
...
for (a in nlevs) {# {{{
	k <- k+1
	datA[,treat.name] <- a
	Xa <- model.matrix(formulanc,datA,xlev=xlev)

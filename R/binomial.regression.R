...
binregTSR <- function(formula,data,cause=1,time=NULL,
		      ...
		      augmentR0=NULL,treat.model0=~+1, 
		      augmentR1=NULL,treat.model1=~+1, 
		      augmentC=NULL, cens.model=~+1,
		      estpr=c(1,1),
	   offset=NULL,weights=NULL,cens.weights=NULL,se=TRUE,beta=NULL,
...
cid <- data$Count1__
dataR0  <- subset(data,Count1__==1)
## last record with statusR==1
dataR1  <- subset(data,response__==1 & rid__==1)
###tail(dataR1); dtable(dataR1,~A0.f+A1.f)
...
if (!is.factor(treatvar1))  treatvar1 <- as.factor(treatvar1) 
...
Response.name <- NA
if (length(all.vars(treat.model1)) >=3) Response.name <- all.vars(treat.model1)[2]
if (!is.na(Response.name)) {
...
for (v in seq(nc))  {
	...
	j <- combo[v,]
	dataij <- dataiid
###	dataij[,treat.name0] <- i
### 	dataij[,treat.name1] <- j
###	dataij$strataij <- strata(i,j)
	pA1j <- rep(1,nrow(dataij))
###     pA1j[fit1[[k]]$id] <- fitt1[[k]]$pal[,j]
	## possibly extend 
	if (estpr[2]==1) pA1j[fit1$id] <- fit1$pA else pA1j[fit1$id] <- 0.5
###	cbind(pA1j,dataij$A0,dataij$A1,i,j,response)
###	pA0i <- fitt0$pal[,i]
	pA0i <- fitt0$pA
	if (estpr[1]==0) pA0i <- 0.5
###	names(fitt0)
###	cbind(fitt0$ppp,fitt0$pal,fitt0$spp,A0)
	...
	DW0 <- W*pA0i
	mm <- cbind(A1,responsetype,A1j,pA1j,W,Y)
###        print(j); print(mm); print(summary(mm));
	...
	iid0 <- sumstrata(iid0,dataiid$id__-1,nid)
	h0 <- DW0*Y
        DaPsia <-  apply(fitt0$DPai*h0,2,sum,drop=FALSE)
        iidpala0 <- c(DaPsia %*% t(fitt0$iidalpha))
        ddR1 <- (A1j)*Y*(A0==i)/pA0i
        ddR1 <- ddR1[response>=1]
###     DaPsia <-  apply(fitt1[[k]]$DPai*c(ddR1),2,sum,drop=FALSE)
###     iidpala <- c(DaPsia %*% t(fitt1[[k]]$iidalpha))
###	iidpala1 <- rep(0,nid)
###	iidpala1[fitt1[[k]]$id] <- iidpala
        DaPsia <-  apply(fit1$DPai*c(ddR1),2,sum,drop=FALSE)
        iidpala <- c(DaPsia %*% t(fit1$iidalpha))
	iidpala1 <- rep(0,nid)
	iidpala1[fit1$id] <- iidpala
        ###
	...
	if (!is.null(augmentR0)) {# {{{
	      ...
	      h0 <- DW0*Y - as.matrix(cbind(1,dataij[,varR0])) %*% coef(llR0)[-1]
              DaPsia <-  apply(c(fitt0$DPai)*(A0==i)*h0,2,sum,drop=FALSE)
	      iidpala0 <- c(DaPsia %*% t(fitt0$iidalpha))
	...
	if ((!is.null(augmentR0)) & (!is.null(augmentR1)) ) {# {{{
	      ...
	      h1 <-  Y - h11
              ...
              ###
              DaPsia0 <-  apply(fitt0$DPai*(A0==i)*h0,2,sum,drop=FALSE)
...
###        else formulaC <- as.formula(paste("Surv(", varss[1], ",", varss[2], ",", varss[3], "==", cens.code, ")~."))

...
##' 
##' @aliases tie.breaker recmarg recurrentMarginalIPCW recurrentMarginalAIPCW 
##' @export
...
###     gamma <- .Call("CubeVec",matrix(c(varZ),nrow=1),matrix(apply(covts,2,sum),nrow=1),1,PACKAGE="mets")$XXbeta
###     gamma <- c(gamma)
###     gamma[is.na(gamma)] <- 0
###     gamma[gamma=Inf] <- 0
###     augment <- sum(apply(gamma*t(cr2$U[timeb,1+1:nterms,drop=FALSE]),2,sum))/nid
...
simMultistateII <- function(n,cumhaz,cumhaz2,death.cumhaz=NULL,
		    rr=NULL,rr2=NULL,rd=NULL,rd2=NULL,
		    gap.time=FALSE,max.recurrent=100,dhaz=NULL,
		    dependence=0,var.z=0.22,cor.mat=NULL,cens=NULL,...) 
  {# {{{
...
### death time simulated
  if (!is.null(death.cumhaz)) {
	  timed   <- timereg::simrchaz(cumhazd,zd,cens=cens)
	  tall$dtime <- timed$time
	  tall$fdeath <- timed$status
  } else { 
	  tall$dtime <- max.time; 

...
##' 		augmentR1=~X11+X12+TR,augmentR0=~X01+X02,augmentC=~A01+A02+X01+X02+A11t+A12t+X11+X12+TR,
##' 		response.code=2)
##' summary(bb) 
##' @export
binregTSR <- function(formula,data,cause=1,time=NULL,
      ...
      return.dataw=0,pi0=0.5,pi1=0.5,cens.time.fixed=1,...)
{# {{{
  cl <- match.call()# {{{
...
for (v in seq(nc))  {
	...
	W1k <- ((response==0)+(response!=0)*(A1j)/pi1)
###	cbind(W1k,A1,response)
...
rownames(riskG) <- rownames(riskG0) <- rownames(riskG1) <- rownames(riskG01) <- rnames
colnames(riskG) <- colnames(riskG0) <- colnames(riskG1) <- 
...
### CC  <- rexp(n)*ce* exp((A0-1)*betac[1]+(A1-1)*betac[2])
 ###
  ...
  status <- (TT<cc)*1
  ###  
  data <- data.frame(time=time,T10=T10,Tt11=Tt11,Tt12=Tt12,TT=TT,status=status,A1=A1,A0=A0,R1=R1,R2=R2,R=(A0==1)*R1+(A0==2)*R2,id=1:n,TR=(A0==1)*TR1+(A0==2)*TR2,Resp=(A0==1)*R1+(A0==2)*R2)

...
##' out <- resmeanIPCW(Event(time,cause!=0)~tcell+platelet+age,bmt,cause=1,time=50,cens.model=~strata(platelet),model="exp")
##' summary(out)
##'
##' ### same as Kaplan-Meier for full censoring model 
##' bmt$int <- with(bmt,interaction(tcell,platelet))
##' out <- resmeanIPCW(Event(time,cause!=0)~-1+int,bmt,cause=1,time=30,cens.model=~strata(platelet,tcell),model="lin")
##' summary(out)

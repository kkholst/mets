...
- theta.des:
  ...
  Design for dependence parameters; when pairs are given, this could be
  a (pairs) \\\times\\ (number of parameters) \\\times\\ (max number
...
The dependence can be modelled via a regression structure for the
independent gamma distributed random effects and their variances that
may depend on cluster covariates. So: \$\$ \theta = h( z_j^T \alpha)
\$\$ where \\z\\ is specified by `theta.des`. The link function can be
the exponential when `var.link=1`.
